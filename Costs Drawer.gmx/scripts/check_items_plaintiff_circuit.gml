/// check_items_plaintiff_circuit()
// CHECKS THE LIST
// REMOVES THE FOLLOWING ITEMS FROM THE LIST
...
var sortList = global.sortingListCCPlaintiff;
...
var item = lastPage_VATNotRecoverable
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"V.A.T. not otherwise recoverable");
    ds_list_delete(list,pos);
}

var item = item_CC_attendingToServeProceedingsOnDefendant;
if (instance_exists(item))
{
...
var item = item_CCattendingToSettleSummons;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Attending counsel to settle Personal Injury Summons");
    ds_list_delete(list,pos);
}
...
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Attending to serve proceedings on the Defendant");
    ds_list_delete(list,pos);
}

var item = item_CC_preparationOfBriefForCounselForSettlementNegotiations;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Brief for Counsel to attend for settlement negotiations");
    ds_list_delete(list,pos);
}

var item = item_CC_attendingLawLibraryForNegotiations;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Attending Law Library for negotiations");
    ds_list_delete(list,pos);
}
...
var item = item_CC_personalInjurySummons;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on Personal Injury Summons");
    ds_list_delete(list,pos);
}

var item = item_CC_stampOnPersonalInjurySummons;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Stamp on Personal Injury Summons");
    ds_list_delete(list,pos);
}
...
var item = item_CC_receivedNoticePursuantToSection11;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Received Notice pursuant to Section 11 - Civil Liability and Courts Act 2004");
    ds_list_delete(list,pos);
}

var item = item_CC_feeOnRepliesToSection11Notice;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on replies to Notice pursuant to Section 11 - Civil Liability and Courts Act 2004");
    ds_list_delete(list,pos);
}

var item = item_CC_receivedNoticeForParticularsFromDefendant;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Received Notice for Particulars from Defendant");
    ds_list_delete(list,pos);
}

var item = item_CC_feeOnRepliesToNoticeForParticulars;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Received Notice for Particulars from Defendant");
    ds_list_delete(list,pos);
...
var item = item_CC_notificationOfMotionForJudgement;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Notification of Motion for Judgement");
    ds_list_delete(list,pos);
}
...
var item = item_CC_consentToLateFilingOfDefence;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Consent to late filing of Defence");
    ds_list_delete(list,pos);
}

var item = item_CC_warningLetterPriorToNoticeOfTrial;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Warning letter prior to issue of Notice of Trial");
    ds_list_delete(list,pos);
}

var item = item_CC_feeOnNoticeOfTrialEngrossingAndCopies;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on Notice of Trial engrossing and copies");
    ds_list_delete(list,pos);
}

var item = item_CC_BookOfPleadingsAndAttendingToFile;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Book of Pleadings and attending to file");
    ds_list_delete(list,pos);
}

var item = item_CC_feeOnNoticeToProduceAndCopyToServe;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on Notice to Produce and copy to serve");
    ds_list_delete(list,pos);
}

var item = item_CC_feeOnCaseForProofs;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on case for proofs");
    ds_list_delete(list,pos);
...
var item = item_CC_instructionsForScheduleOfWitnesses;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Instructions for Schedule of Witnesses");
    ds_list_delete(list,pos);
}

var item = item_CC_feeOnPlaintiffsScheduleOfWitnesses;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on Schedule of Witnesses");
    ds_list_delete(list,pos);
}


var item = item_CC_instructionsForScheduleofMedicalReports;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Instructions for Schedule of Medical Reports");
    ds_list_delete(list,pos);
}

var item = item_CC_feeOnPlaintiffsScheduleOfMedicalReports;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on Schedule of Medical Reports");
    ds_list_delete(list,pos);
}


var item = item_CC_feeOnPreparationOfBriefToCounselForTrial
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on brief to counsel for Trial of Action");
    ds_list_delete(list,pos);
}
...
var item = item_CC_instructionsForConsultationPriorToHearing;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Instructions for consultation prior to hearing");
    ds_list_delete(list,pos);
}


var item = item_CC_feeOnConsultationPriorToHearing;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Fee on consultation prior to hearing");
    ds_list_delete(list,pos);
}


var item = item_CC_attendingCourtForHearing;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Attending court for hearing...");
    ds_list_delete(list,pos);
}

var item = item_CC_attendingForTheOrder;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Attending for the order");
    ds_list_delete(list,pos);
...
var item = lastPage_CC_stampOnCertificate;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Stamp on Certificate of Taxation");
    ds_list_delete(list,pos);
}

var item = item_CC_stampOnNoticeOfTrial;
if (instance_exists(item))
{
    var pos = ds_list_find_index(list,"Stamp on Notice of Trial");
    ds_list_delete(list,pos);
